                 ...
                 const DBusSignatureIter * sig_iter)
{
    ...
    while (enif_get_list_cell(env, term_list, &term, &term_list))
    {
        ...
        // complete type.
        dbus_signature_iter_recurse(sig_iter, &sub_sig_iter);
        ret = iter_append_arg(env, term, &sub_appender, &sub_sig_iter, NULL);
        if (!ret)
        {
            break;
        }
                   ...
                   const DBusSignatureIter * sig_iter)
{
    ...
    for (int i = 0; i < arity; i++)
    {
        dbus_bool_t more;
        ret =
            iter_append_arg(env, entries[i], &sub_appender, &sub_sig_iter, &more);
        if (!ret)
        {
            break;
        }
                       ...
                       const DBusSignatureIter * sig_iter)
{
    ...
    ret = iter_append_arg(env, key, &sub_appender, &sub_sig_iter, NULL);
    if (ret == TRUE)
    {
        ret = iter_append_arg(env, key, &sub_appender, &sub_sig_iter, NULL);
    }
                ...
                const DBusSignatureIter * sig_iter)
{
    ...
    while (enif_map_iterator_get_pair(env, &map_iter, &key, &value))
    {
        ...
        dbus_signature_iter_recurse(sig_iter, &sub_sig_iter);
        ret =
            iter_append_dict_entry(env, key, value, &sub_appender, &sub_sig_iter);
        if (!ret)
        {
            break;
        }
...
ebus_message_append_args(ErlNifEnv * env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    while (more)
    {
        ...
        if (!iter_append_arg(env, arg, &appender, &sig_iter, &more))
        {
            // {error, enomem}
            return enif_make_badarg(env);
        }
